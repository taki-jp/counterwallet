...
function TransactionHistoryItemViewModel(data) {
  ...
  self.dispDescription = function() {
    ...
    function number_to_currency(value, unit) {
      if (unit === undefined) {
        unit = KEY_ASSET.XCP;
      }
    ...
    if (self.RAW_TX_TYPE == 'burns') {
      desc = i18n.t("hist_burn", KEY_ASSET.XCP, number_to_currency(normalizeQuantity(self.DATA['burned']), KEY_ASSET.BTC), number_to_currency(smartFormat(normalizeQuantity(self.DATA['earned']))));
    } else if (self.RAW_TX_TYPE == 'sends') {
      desc = i18n.t("hist_send", number_to_currency(smartFormat(normalizeQuantity(self.DATA['quantity'],
      ...
      desc = i18n.t("hist_sell", number_to_currency(smartFormat(normalizeQuantity(self.DATA['give_quantity'], self.DATA['_give_asset_divisible'])),
        self.DATA['_give_asset_longname'] || self.DATA['give_asset']),
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['get_quantity'], self.DATA['_get_asset_divisible'])),
      ...
      desc = i18n.t("hist_order_match", getAddressLabel(self.DATA['tx0_address']),
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['forward_quantity'], self.DATA['_forward_asset_divisible'])),
        self.DATA['_forward_asset_longname'] || self.DATA['forward_asset']),
        getAddressLabel(self.DATA['tx1_address']),
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['backward_quantity'], self.DATA['_backward_asset_divisible'])),
      ...
      desc = i18n.t("hist_btcpay", number_to_currency(smartFormat(normalizeQuantity(self.DATA['btc_amount'])), KEY_ASSET.BTC));
      ...
      desc = i18n.t("hist_bet", BET_TYPES[self.DATA['bet_type']],
        ...
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['wager_quantity']))),
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['counterwager_quantity']))));
      ...
      desc = i18n.t("hist_bet_match", getLinkForCPData('address', self.DATA['feed_address'], getAddressLabel(self.DATA['feed_address'])),
        ...
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['forward_quantity']))),
        getAddressLabel(self.DATA['tx1_address']),
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['backward_quantity']))));
      ...
      desc = i18n.t("hist_credit_debit", getLinkForCPData('address', self.DATA['address'], getAddressLabel(self.DATA['address'])), tx_type,
        number_to_currency(smartFormat(normalizeQuantity(self.DATA['quantity'], self.DATA['_asset_divisible'])), self.DATA['_asset_longname'] || self.DATA['asset']));
